...
const StudentPaymentsPanel = ({ studentId }) => {
  ...
  useEffect(() => {
    if (!studentId) {
      setLoading(false);
      setError('Payments are not available yet.');
      return;
    }
    ...
    const fetchPayments = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`/api/student_payments.php?student_id=${studentId}`);
  ...
  if (loading) {
    return (
      <div style={styles.loadingWrap}>
        ...
        <div style={{ color: '#475569', fontWeight: 600 }}>Loading payments...</div>
      </div>
    );
  }
  ...
  const stats = [
    { label: 'Total Paid Months', value: data?.total_paid_months ?? 0 },
    { label: 'Total Unpaid Months', value: data?.total_unpaid_months ?? 0 },
    { label: 'Total Outstanding Amount', value: currency(data?.total_outstanding_amount ?? 0) },
  ];
      ...
      <div style={styles.tableWrap}>
        ...
        {Array.isArray(data?.history) && data.history.length > 0 ? (
            ...
            <table style={styles.table}>
              ...
              <tbody>
                {data.history.map((row, index) => {
                  ...
                  return (
                    <tr key={`${row.month}-${row.amount}-${index}`}>
                      ...
                      <td style={styles.td}>{row.payment_date || 'N/A'}</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          <div style={{ padding: '20px' }}>
            <div style={styles.empty}>No payment records available yet.</div>
          </div>
        )}
      </div>
    </div>
