...
class ExaminationController
{
    ...
    private function ensureDefaultExams(): void
    {
        $grades = ['Grade 6', 'Grade 7', 'Grade 8', 'Grade 9', 'Grade 10', 'Grade 11'];
        $terms = ['Term 1', 'Term 2', 'Term 3'];
        $subjects = $this->getStandardSubjects();
        $currentYear = (int) date('Y');

        $examStmt = $this->db->prepare("SELECT id FROM exams WHERE grade = ? AND term = ? LIMIT 1");
        $createExamStmt = $this->db->prepare("INSERT INTO exams (exam_name, exam_date, grade, term) VALUES (?, ?, ?, ?)");
        $subjectStmt = $this->db->prepare("INSERT IGNORE INTO exam_subjects (exam_id, subject) VALUES (?, ?)");

        foreach ($grades as $grade) {
            foreach ($terms as $index => $term) {
                $examStmt->execute([$grade, $term]);
                $examId = (int) $examStmt->fetchColumn();

                if ($examId <= 0) {
                    $month = str_pad((string) (($index * 4) + 3), 2, '0', STR_PAD_LEFT);
                    $examDate = sprintf('%d-%s-15', $currentYear, $month);
                    $examName = sprintf('%s %s Examination', $grade, $term);
                    $createExamStmt->execute([$examName, $examDate, $grade, $term]);
    ...
    public function getAdminResults(): void
    {
        ...
        if ($examId > 0) {
            ...
            $exam = $this->getLatestExamByGradeAndTerm($grade, $term);
            if (!$exam) {
                $exam = [
                    'id' => 0,
                    'exam_name' => 'No exam created yet',
                    'exam_date' => '',
                    'grade' => $grade,
        ...
        $marks = $examId > 0 ? $this->getMarksForExam($examId) : [];
    ...
    private function getLatestExamByGradeAndTerm(string $grade, string $term): ?array
    {
        $variants = $this->buildGradeVariants($grade);
        $placeholders = implode(',', array_fill(0, count($variants), '?'));
        $params = $variants;
        $sql = "SELECT id, exam_name, exam_date, grade, term FROM exams WHERE grade IN ({$placeholders})";
        ...
        $sql .= " ORDER BY exam_date DESC, id DESC LIMIT 1";
    ...
    private function getStandardSubjects(): array
    {
        return ['Tamil', 'Maths', 'Science', 'Religion', 'English', 'Civics', 'History', 'Geography', 'ICT', 'Health Science', 'Sinhala', 'Commerce'];
    }
