...
const AdminExaminations = () => {
  ...
  const loadExams = async () => {
    try {
      ...
      const response = await axios.get('/api/admin/examinations');
  ...
  useEffect(() => {
    ...
    const loadResults = async () => {
      try {
        ...
        const response = await axios.get(`/api/admin/examinations/results?exam_id=${selectedExamId}`);
  ...
  const handleCreateExam = async (event) => {
    ...
    try {
      const response = await axios.post('/api/admin/examinations', formValues);
...
<div className="dashboard-edit-card" style={{ marginBottom: 24 }}>
          ...
          <div style={styles.toolbar}>
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label htmlFor="exam-results-exam">Select Exam</label>
              <select
                id="exam-results-exam"
                value={selectedExamId}
                onChange={(e) => setSelectedExamId(e.target.value)}
                disabled={loadingExams || filteredExams.length === 0}
              >
                {filteredExams.length === 0 ? (
                  <option value="">No exams for {selectedGrade} - {selectedTerm}</option>
                ) : (
            ...
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label htmlFor="exam-results-sort">Sort Students By</label>
              <select
                id="exam-results-sort"
                value={sortMode}
            ...
            <div className="form-group" style={{ marginBottom: 0 }}>
              <label>Classes Shown</label>
              <input type="text" readOnly value={processedClasses.length} />
            </div>
          </div>
        </div>
        ...
        {loadingResults ? (
          ...
          <div className="dashboard-empty">Create or select an exam to view results for {selectedGrade} {selectedTerm}.</div>
        ) : !results ? (
          <div className="dashboard-empty">No result data available yet.</div>
        ) : (
          <>
